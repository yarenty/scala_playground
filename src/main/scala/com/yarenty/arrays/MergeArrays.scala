...
Think about edge cases! What happens when we've merged in all of the elements from one of our arrays but we still have
elements to merge in from our other array?
...
We can do better. With this algorithm, we're not really taking advantage of the fact that the input arrays are
themselves already sorted. How can we save time by using this fact?

A good general strategy for thinking about an algorithm is to try writing out a sample input and performing
the operation by hand. If you're stuck, try that!

Since our arrays are sorted, we know they each have their smallest item in the 0th index. So the smallest item
overall is in the 0th index of one of our input arrays!
...
We need to make sure we don't try to put that 1 in merged_array again. We should mark it as "already merged"
somehow. For now, we can just cross it out:
...
merged_array = [1,  x,  x,  x,  x,  x]
Now to get our next element we can use the same approach we used to get the 0th element—it's the smallest of
the earliest unmerged elements in either array! In other words, it's the smaller of the leftmost elements in
either array, assuming we've removed the elements we've already merged in.
...
Can you implement this in code?
  ...
  def merge_arrays(my_array, alices_array):
    ...
    return merged_array
Okay, this algorithm makes sense. To wrap up, we should think about edge cases and check for bugs. What edge cases
...
Actually, 3 will always happen. In the process of merging our arrays, we'll certainly exhaust one before we exhaust
the other.
...
We can probably solve all of these at the same time. They're not so different—they all have to do with handling
empty arrays.

To start, we could treat each of our arrays being out of elements as a separate case to handle, in addition to the
2 cases we already have. So we have 4 cases total. Can you code that up?
...
We keep track of a current index in my_array, a current index in alices_array, and a current index in merged_array.
So at each step, there's a "current item" in alices_array and in my_array. The smaller of those is the next one we
add to the merged_array!

But careful: we also need to account for the case where we exhaust one of our arrays and there are still elements
in the other. To handle this, we say that the current item in my_array is the next item to add to merged_array only
if my_array is not exhausted AND, either:
...
the current item in my_array is less than the current item in alices_array
  def merge_arrays(my_array, alices_array):
    ...
    return merged_array
The if statement is carefully constructed to avoid an IndexError. We first check whether or not the arrays are
exhausted, before we index into them (which is where we risk throwing an IndexError). Python's lazy evaluation ↴
then ensures we don't index into our arrays if one is already exhausted.

Complexity
O(n)O(n) time and O(n)O(n) additional space, where nn is the number of items in the merged array.

The added space comes from allocating the merged_array. There's no way to do this "in place" because neither of
our input arrays are necessarily big enough to hold the merged array.
...
In our implementation above, we could avoid tracking current_index_merged and just compute it on the fly by adding
current_index_mine and current_index_alices. This would only save us one integer of space though, which is hardly
anything. It's probably not worth the added code complexity.

Trivia! Python's native sorting algorithm is called Timsort. It's actually optimized for sorting arrays where
subsections of the array are already sorted. For this reason, a more naive algorithm:
...
Bonus
What if we wanted to merge several sorted arrays? Write a function that takes as an input an array of sorted arrays
and outputs a single sorted array with all the items from each array.

 *
 *
 * Created by yarenty on 14/08/2015.
 */
object MergeArrays {


  def merge_arrays(my_array: Array[Int], alices_array: Array[Int]): Array[Int] = {
    //set up our merged_array
    ...
    var current_index_merged = 0
    var is_my_array_exhausted = false
    var is_alices_array_exhausted = false

    while (current_index_merged < merged_array_size) {

      is_my_array_exhausted = current_index_mine >= my_array.size
      is_alices_array_exhausted = current_index_alices >= alices_array.size

      //case: next comes from my array#my array must not be exhausted, and EITHER:
      // 1) alice 's array IS exhausted, or
      ...
      if (!is_my_array_exhausted && (is_alices_array_exhausted ||
        ...
        merged_array(current_index_merged) = my_array(current_index_mine)
        current_index_mine += 1 //case: next comes from alice 's array
      }
      else {
        merged_array(current_index_merged) = alices_array(current_index_alices)
        current_index_alices += 1
  ...
  def main(args: Array[String]) {
    ...
    val my_array = Array(3, 4, 6, 10, 11, 15)
    val alices_array = Array(1, 5, 8, 12, 14, 19)

    val merged = merge_arrays(my_array, alices_array)
