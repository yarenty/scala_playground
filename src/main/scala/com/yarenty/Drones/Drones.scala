...
Alright, we got our runtime down to O(n)O(n). That's probably the best runtime we can getmdash;to find our unique integer we’ll definitely have to look at every integer in the worst case.

But now we've added O(n)O(n) space, for our hash. Can we bring that down?

Well, we could use booleans ↴ as our values, instead of integers. If we see an integer, we'll add it as a key in our hash with a boolean value of True. If we see it again, we'll change its value to False. At the end, our non-repeated order ID will be the one integer with a value of True.
...
If so, we could start with a variable unique_delivery_id set to 00 and run some bitwise operation with that variable and each number in our array. If duplicate integers cancel each other out, then we’d only be left with the unique integer at the end!
